...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
        List<String> articles = Arrays.asList("a" , "an", "the", "and", "but", "for", "nor", "or", "so", "yet", "at", "by", "in", "into", "near", "of", "on", "to", "than", "via") ;
    ...
    public void setAuthor(String author) {
        List<String> authorArr= Arrays.asList(author.split(" "));
        if (author.equals("")){
            System.out.println("Unknown");
            unknownAuthors++;
        }else {
            for (int i = 0; i <authorArr.size() ; i++) {
                author+= capitalize(authorArr.get(i))+" ";
            }
            System.out.println(author.substring(0, author.length()-1));
        }
    ...
    public void setTitle(String title) {
        ...
        if (title.equals(" ")) {
            title = "we will handle this in Shelf class properly";
        }else {
            title=capitalize(arrTitle.get(0));
            for (int i = 1; i < arrTitle.size(); i++) {
                if (isTitleWord(arrTitle.get(i))) {
                    arrTitle.set(i, capitalize(arrTitle.get(i)));
                }

                title +=arrTitle.get(i)+" ";
            }
